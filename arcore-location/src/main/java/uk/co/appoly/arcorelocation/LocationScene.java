...
public class LocationScene {
    ...
    public LocationScene(Activity context, ArSceneView mArSceneView) {
        ...
        startCalculationTask(); // chạy anchorRefreshTask 5 giây anchorsNeddRefresh = true

        deviceLocation = new DeviceLocation(context, this);
        deviceOrientation = new DeviceOrientation(context);
        deviceOrientation.resume(); // lắng nghe vector quay , và tốc độ quay hướng màn hình
        //test();
    }
    ...
    // đưa ra thông báo lên màn hình old, invalid...
...
//        }
    ...
    Location lastDeviceLocation = null;
    final double MinUpdateMarkerAngleRad = 5 * Math.PI / 180;
    ...
    private void refreshAnchorsIfRequired(Frame frame) {
        ...
        for (int i = 0; i < mLocationMarkers.size(); i++) {
            try {
                ...
                int markerDistance = (int) Math.round(LocationUtils.distance(marker.latitude, deviceLocation.currentBestLocation.getLatitude(), marker.longitude, deviceLocation.currentBestLocation.getLongitude(), 0, 0));

                // [20191203    VMio] Fix change marker every time make camera view be slow
                double cosAngleDeviveRotateAroundMarker = 1.0;
                if (lastDeviceLocation != null) {
                    // Estimate angle by https://www.mathsisfun.com/algebra/trig-solving-sss-triangles.html
                    double dstMarkerAndAfterDevice = LocationUtils.distance(marker.latitude, deviceLocation.currentBestLocation.getLatitude(), marker.longitude, deviceLocation.currentBestLocation.getLongitude(), 0, 0);
                    double dstMarkerAndBeforeDevice = LocationUtils.distance(lastDeviceLocation.getLatitude(), marker.latitude, lastDeviceLocation.getLongitude(), marker.longitude, 0, 0);
                    double dstBeforeAndAfterDevice = LocationUtils.distance(lastDeviceLocation.getLatitude(), deviceLocation.currentBestLocation.getLatitude(), lastDeviceLocation.getLongitude(), deviceLocation.currentBestLocation.getLongitude(), 0, 0);
                    cosAngleDeviveRotateAroundMarker = (dstMarkerAndAfterDevice * dstMarkerAndAfterDevice + dstMarkerAndBeforeDevice * dstMarkerAndBeforeDevice - dstBeforeAndAfterDevice * dstBeforeAndAfterDevice) / (2 * dstMarkerAndAfterDevice * dstMarkerAndBeforeDevice);
                    double angleRadian = Math.acos(cosAngleDeviveRotateAroundMarker);
                    // Angle
                    if (angleRadian < MinUpdateMarkerAngleRad)
                        continue;

                    // [20191203    PhuongTQ] move toward marker far enough then update marker
                    double distanceRatio = dstMarkerAndAfterDevice < dstMarkerAndBeforeDevice ? dstMarkerAndAfterDevice/dstMarkerAndBeforeDevice : dstMarkerAndBeforeDevice/dstMarkerAndAfterDevice;
                ...
                if (markerDistance > marker.getOnlyRenderWhenWithin()) {
                    ...
                    Log.i(TAG, "Not rendering. Marker distance: " + markerDistance + " Max render distance: " + marker.getOnlyRenderWhenWithin());
                    continue;
                }

                // Tính góc
                float bearing = (float) LocationUtils.bearing(deviceLocation.currentBestLocation.getLatitude(), deviceLocation.currentBestLocation.getLongitude(), marker.latitude, marker.longitude);
                ...
                Log.d(TAG, "currentDegree " + deviceOrientation.getOrientation() + " bearing " + bearing + " markerBearing " + markerBearing + " rotation " + rotation + " distance " + markerDistance);
                ...
                Anchor newAnchor = mSession.createAnchor(frame.getCamera().getDisplayOrientedPose().compose(translation).extractTranslation());
        ...
        lastDeviceLocation = deviceLocation.currentBestLocation;
    ...
    public void resume() {
        deviceOrientation.resume(); // lắng nghe vector quay , và tốc độ quay hướng màn hình
        deviceLocation.resume(); // update location : update sau 5 giây và 1 m di chuyển
    }
